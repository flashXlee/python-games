...
def draw():
    ...
    if game_over:
        display_message("GAME OVER LOSER","TRY AGAIN NOOB")
    elif game_complete:
        display_message("YOU WON","YOU ARE A PRO, MR.PRO")
    else:
        for star in stars:
            star.draw()
...
def get_colours_to_create(numbers_of_extra_stars):
    colours_to_create = ["red"]
    for i in range(0, numbers_of_extra_stars):
...
def display_message(win,lose):
    screen.draw.text(win, fontsize=60, center =CENTRE, color=FONT_COLOUR)
    screen.draw.text(lose, fontsize=30, center=(CENTRE_X, CENTRE_Y + 30), color=FONT_COLOUR)
